...
run_dir = "C:/Users/James.Thorson/Desktop/Git/Spatio-temporal-models-for-ecologists/Chap_9/"
...
Ice <- data.frame(
               Longitude = rep( as.vector(longitudeSubset), length(datatime) ),
               Latitude = rep( as.vector(latitudeSubset), length(datatime) ),
               Year = rep( year(datatime), each=prod(dim(latitudeSubset)) ),
               Ice = as.vector(seaiceCDR)
             )
Ice = na.omit(Ice)
write.csv( Ice, paste0(run_dir,"Ice.csv") )
